...
| ~~`vite-plugin-dts`가 일부 내부 컴포넌트 `.d.ts`에 깨진 상대경로(`'../../../../core/src'` 등)를 남김~~ | F2-4 작업 중 발견 (2026-06-30) | `vite.lib.config.ts`의 `@vuepkg/core`/`@vuepkg/ui` alias(원시 src 지정) 때문에 `vite-plugin-dts`가 그 경로를 그대로 `.d.ts` import에 박아넣음. **재진단(2026-07-02)**: 이전에 "alias 제거 → `@vuepkg/ui` CSS가 `style.css`에서 통째로 빠짐"이라는 회귀로 기록되어 되돌려졌으나, 실제로는 `@vuepkg/ui`가 이미 자체 `build:lib`으로 `dist/style.css` + `exports["./style.css"]`를 갖추고 있었음 — 이전 시도가 alias만 제거하고 그 CSS를 명시적으로 import하지 않아서 난 회귀였을 뿐, alias-CSS 결합이 근본적으로 분리 불가능한 게 아니었음 | 현재는 공개 엔트리(`dist/components/calendar/index.d.ts`)의 타입 그래프 밖이라 일반 소비자에게는 무해했음. `MonthOverflowPopover.vue.d.ts` 같은 비공개 컴포넌트를 deep-import하거나, F3-2(`vue-component-meta` 자동 문서화)에서 전 컴포넌트를 introspect할 때 문제될 수 있었음 | ✅ 해결 (2026-07-02, SRV-P1-02) — `vite.lib.config.ts`·`vite.config.ts`(데모)에서 `@vuepkg/core`/`@vuepkg/ui` alias 제거(workspace symlink 경유로 정상 해석), `ScheduleCalendar.vue`에 `@import '@vuepkg/ui/style.css'` 명시 추가. `turbo.json`의 `dev` task에 `dependsOn: ["^build:lib"]` 추가(alias 제거로 ui/core dist 선빌드가 필요해짐). dts 클린 확인, style.css 크기·클래스 동일 확인, Vitest 290건·E2E 142건·size-limit 전부 통과 |
...
| F4-5 | Recurring Event (반복 일정) — 신규 | 🔴 | ✅ **완료 (2026-07-01)** — `RecurrenceRule`(freq/interval/byWeekday/count/until/exceptions) + `expandRecurringSchedules`, `useCalendar`가 뷰별 표시 범위 내에서 자동 펼침. `ScheduleFormModal` 반복 UI. v1은 시리즈 단위 수정/삭제만 지원 |
...
| calendar 번들 사이즈 (brotli) | index.js 15.35KB + style.css 5.04KB (F4-5 반복 일정 반영, size-limit budget 16KB/6.5KB 내) | core 분리로 ↓ | budget 내 유지 — 도메인 기능 추가에도 size-limit CI(F4-9)로 가드. index.js가 budget의 96%까지 근접 — F4-6/F4-7 착수 전 budget 상향 또는 코드 분할 검토 필요 |
| 문서 커버리지 | docs/ 내부 문서 + theming.md + `@vuepkg/ui` README + CONTRIBUTING.md(F4-10) | 사이트 + 자동 API | 전 도메인 기능 라이브 데모(Timeline 시연 포함) |
...
| 테스트 | Vitest 393(core 70 + ui 76 + calendar 247) / E2E 기능 **134**(CI·`test:e2e:ci`) + 시각 **8**(수동·`test:e2e:visual`) | 패키지별 유지·증가 | 도메인 기능별(Timeline 등) 신규 테스트 스위트 |
