...
프로젝트의 방향성(어떤 기능을 왜 만드는지)은 [docs/dev/roadmap.md](docs/dev/roadmap.md)를 먼저 읽어보세요. "calendar가 실제로 쓰는 것만 추출한다", "범용 UI 프레임워크는 목표가 아니다" 같은 원칙이 PR 리뷰 기준이 됩니다.
...
- `docs/dev/roadmap.md` Phase 4의 🔴 높음 난이도 항목(F4-5 반복 일정, F4-6 Timeline 등)처럼 설계 여지가 큰 기능
...
RFC가 합의되면 `docs/dev/roadmap.md`(또는 해당 백로그 문서)에 항목을 등록한 뒤 구현을 시작합니다.
...
- [docs/dev/roadmap.md](docs/dev/roadmap.md) — 비전·Phase별 로드맵·달성률·기술 부채
